"""Moduuli joka vastaa Databasesta"""
import sqlite3
...
class Database:
    ...
    def add_reference_to_database(self, reference):
        ...
        cursor.execute(
            ...
            (
                ...
                reference.fields.get("howpublished"),
                reference.fields.get("note"),
                reference.fields.get("school"),
                reference.fields.get("address"),
                reference.fields.get("month"),
            ),
        )
        ...
        if reference.tags:
            for tag in reference.tags:
                cursor.execute(
                    "INSERT INTO bibtex_tags (reference_id, tag) VALUES (?, ?)",
                    (reference_id, tag),
                )
    ...
    def get_all_references(self):
        ...
        for row in references_rows:
            # Hae tagit
            cursor.execute(
                "SELECT tag FROM bibtex_tags WHERE reference_id = ?", (row[0],)
            )
